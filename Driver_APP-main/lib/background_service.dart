  ...
  Future<void> processQueue() async {
    try {
      ...
      for (final key in keys) {
        ...
        debugPrint('BackgroundService.processQueue: serverUrl=$serverUrl auth=${authToken != null} key=$key');

        if (locationPoint == null) continue;
        if (serverUrl == null || authToken == null) {
          debugPrint('BackgroundService.processQueue: serverUrl or authToken is null, stopping processing');
          return;
        }
        try {
          ...
          debugPrint('BackgroundService.processQueue: POST status=${response.statusCode}');
  ...
  service.on('startTracking').listen((event) async {
    try {
      ...
      if (authToken == null || apiBaseUrl == null || busId == null) {
        debugPrint('BackgroundService.startTracking: missing param busId/apiBaseUrl/authToken -> $event');
        return;
      }
    ...
    try {
      positionStream = Geolocator.getPositionStream(locationSettings: locationSettings).listen(
          ...
          debugPrint('BackgroundService.positionStream: got position; serverUrl=$serverUrl auth=${authToken != null}');

          try {
            if (serverUrl == null || authToken == null) {
              debugPrint('BackgroundService.positionStream: serverUrl/authToken null; queuing point');
            ...
            debugPrint('BackgroundService.positionStream: POST status=${response.statusCode}');
            if (response.statusCode >= 200 && response.statusCode < 300) {
              await processQueue();
            } else {
              await locationBox?.add(locationPoint);
        ...
        onError: (error) {
          debugPrint('BackgroundService.positionStream: stream error: $error');
        },
      );
    } catch (e, st) {
      ...
      periodicFlushTimer = Timer.periodic(const Duration(seconds: 30), (_) async {
        await processQueue();
      });
    }
    } catch (e, st) {
