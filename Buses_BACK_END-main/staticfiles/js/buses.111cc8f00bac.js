document.addEventListener('DOMContentLoaded', function () {
            ...
            if (buses && buses.length > 0) {
                buses.forEach(bus => {
                    ...
                    if (bus.bus_id === editingBusId) {
                        ...
                        populateBusLinesDropdown(busLineSelect, bus.bus_line);
                    } else {
    ...
    async function addNewBus() {
        // This logic remains the same, but is called by the event listener now
        ...
        try {
            const response = await fetch('/api/buses/', {
                ...
                body: JSON.stringify({
                    ...
                    bus_line_id: busLineId ? parseInt(busLineId, 10) : null
                })
            });
            if (!response.ok) {
